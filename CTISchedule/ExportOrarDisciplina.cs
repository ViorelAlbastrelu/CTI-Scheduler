...
{
    public partial class ExportOrarDisciplina : UserControl
    {
        ...
        public ExportOrarDisciplina(string name)
        {
            InitializeComponent();
            string query = $@"select Zile.Nume, Modul.OraStart, Disciplina.Nume, Profesor.Nume, Activitate.Nume, Sala.Nume, Grupa.Nume, Subgrupa.Nume
                            from Profesor
                    ...
                    for (int i = 0; i < count; i++)
                    {
                        for (int j = 0; j < zile.Length; j++)
                        {
                            for(int k=0; k < oraStart.Length; k++)
                            {
                                ...
                                {
                                    //i is the column number brought from the querry select. Ex: i = 0 is Zile.Nume
                                    ...
                                    {
                                        if (reader.GetValue(4).ToString() == "Seminar")
                                        {
                                            if (i >= 3 && i < 7 ) //Gets everything from Profesor.Nume excluding the subgroup
                                            {
                                        ...
                                        if (reader.GetValue(4).ToString() == "Laborator")
                                        {
                                            if (i >= 3 && i != 6) //Gets everything from Profesor.Nume excluding the group
                                            {
                                                Console.WriteLine(reader.GetValue(i));
                                                labels[z].Text += reader.GetValue(i) + "\n";
                                                if (i == count - 1)
                                        ...
                                        if (reader.GetValue(4).ToString() == "Curs")
                                        {
                                            if (i >= 3 && i < 6) //Gets everything from Profesor.Nume excluding the group and the subgroup
                                            {
                                                Console.WriteLine(reader.GetValue(i));
                                                labels[z].Text += reader.GetValue(i) + "\n";
                                                if (i == count - 1)
